...
import {reqRentalProducts, reqAddRentalProduct, reqChangeRentalDepository} from "../../api";
...
export default class Rentalwarehousing extends Component {
    state = {
    ...
    getProducts = async (currentPage) => {
        ...
        let result
        result = await reqRentalProducts({currentPage, searchCate, searchAddress, searchName})
    ...
    changeDepository = (product) => {
        this.showChange()
        // console.log(product)
        const {depositoryId, rentalName, stock} = product
        console.log(depositoryId, rentalName, stock)
        this.setState({
            onChangeDepositoryId: depositoryId,
            onChangeDepositoryName: rentalName,
    ...
    handleChangeDepository = async () => {
        ...
        Modal.confirm({
            ...
            onOk: async () => {
                ...
                const rentalDepositoryId = onChangeDepositoryId
                const rentalDepositoryStock = changeStock + onChangeDepositoryStock
                if (rentalDepositoryStock < 0) {
                    // message.error('减少库存后数量不能低于0')
                    message.error('stock cannot be under 0')
                    return
                }
                console.log(changeStock)
                const result = await reqChangeRentalDepository(rentalDepositoryId, rentalDepositoryStock)
                if (result.code === 200) {
        ...
        // const result = await reqChangeDepository(rentalDepositoryId, rentalDepositoryStock)
        // if (result.code === 200) {
    ...
    render() {
        ...
        const NormalLoginForm = () => {
            const onFinish = async (values) => {
                ...
                const result = await reqAddRentalProduct(
                    productName,
                    productImage,
                    productUnit,
                    productCate,
